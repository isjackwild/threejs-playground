...
const geom = new THREE.PlaneGeometry( FLOOR_SIZE, FLOOR_SIZE, 100, 100 );
...
const material = new THREE.ShaderMaterial({
	vertexShader: NOISE_3D + VERTEX_SHADER,
	fragmentShader: NOISE_3D + FRAGMENT_SHADER,
	uniforms: {
		uTouch: { value: new THREE.Vector3() },
		uTime: { value: noiseTime },
		uRes: { value: new THREE.Vector2(FLOOR_SIZE, FLOOR_SIZE) }
	},
});
...
const onIntersect = (intersects) => {
	...
	material.uniforms.uTouch.value.x = vec.x;
	material.uniforms.uTouch.value.y = vec.z * -1;
	material.uniforms.uTouch.value.z = vec.y;
